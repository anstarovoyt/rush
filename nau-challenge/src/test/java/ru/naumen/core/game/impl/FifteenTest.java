...
public class FifteenTest
{
    ...
    @Test
    public void correctInputDoNotCauseErrorMessage() {
        Fifteen game = new Fifteen();
        game.input("");
        assertThat(game.output(), not(containsString("Некорректный ввод")));
    }

    @Test
    public void notIntegerMovesAreConsideredAsInccorrect() {
        Fifteen game = new Fifteen();
        game.input("test");
        assertThat(game.output(), containsString("Некорректный ввод"));
